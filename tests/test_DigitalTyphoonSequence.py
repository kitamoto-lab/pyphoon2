...
class TestDigitalTyphoonSequence(TestCase):
    ...
    def test_process_track_data_track_entries_should_be_assigned_to_correct_images(self):
        ...
        self.assertTrue(np.array_equal(image1.track_array(), 
                                       np.array([2008., 4., 13., 0., 2., 8.6, 128., 1006., 0., 0., 0., 0., 0., 0., 0., 0., 0.])))

        image2 = test_sequence.get_image_at_idx(1)
        self.assertTrue(np.array_equal(image2.track_array(),
                                       np.array([2008., 4., 13., 1., 2., 8.64, 127.71, 1005.7 ,0., 0., 0., 0., 0. ,0., 0., 0., 1.])))

        image3 = test_sequence.get_image_at_idx(2)
        self.assertTrue(np.array_equal(image3.track_array(),
                                       np.array(
